...
namespace Supercent.Common.FlowField
{
    ...
    internal static class FlowFieldCoarseTopologyBaker
    {
            ...
            out string error)
        {
            ...
            neighborMasks = null;
            error = string.Empty;
            if (!fineGrid.IsValid || surface == null || !surface.HasValidData)
            {
                error = "Fine Grid 또는 Surface Bake가 유효하지 않습니다.";
                return false;
            }

            coarseMultiplier = Mathf.Max(2, coarseMultiplier);
            walkableRatioThreshold = Mathf.Clamp01(walkableRatioThreshold);
