...
{
    /// <summary>
    /// Managed + optional Native backing for FlowField rebuild.
    /// Native arrays are allocated on demand for Burst jobs and disposed with the workspace.
    /// </summary>
    internal sealed class FlowFieldWorkspace : IDisposable
    {
        ...
        public void EnsureNative(int cellCount, Allocator allocator = Allocator.Persistent)
        {
            if (cellCount <= 0)
                return;

            if (_nativeAllocated && NativeCosts.IsCreated && NativeCosts.Length == cellCount)
                return;
