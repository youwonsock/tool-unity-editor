...
namespace Supercent.Common.FlowField
{
    internal static class FlowFieldSurfaceBaker
    {
        ...
        public static bool TryBake(
            ...
            out FlowFieldSurfaceBakeResult result,
            out string error)
        {
            result = null;
            error = string.Empty;
            if (!settings.IsValid)
            {
                error = "Grid, Ground Layer 또는 Bake Bounds 설정이 유효하지 않습니다.";
                return false;
            }
            ...
            if (!FlowFieldBakeBoundsUtility.TryValidateCellCount(
                    ...
                    out int cellCount))
            {
                error = $"Cell Count가 상한({FlowFieldBakeBoundsUtility.MaxCellCount:N0})을 초과합니다. "
                    + $"현재 {grid.Width} × {grid.Depth} cells입니다. Bake Bounds 또는 Cell Size를 줄이세요.";
                return false;
            }

            Physics.SyncTransforms();
            result = new FlowFieldSurfaceBakeResult(cellCount);
            ...
            {
                error = "Bake Bounds 범위에서 이동 가능한 Ground Collider를 찾지 못했습니다.";
                result = null;
                return false;
            }
