...
namespace Common.FlowField
{
    internal sealed class FlowFieldModifierRegistry
    {
        internal sealed class Entry
        {
            ...
            // Surface allocates this lazily. Volume never creates it, so the
            // common registration record does not own Surface cell arrays.
            internal FlowFieldSurfaceModifierState SurfaceState;
        ...
        internal bool Contains(IFlowFieldVectorModifier modifier)
            => Find(modifier) != null;
        ...
        internal FlowFieldModifierRegistryResult MarkDirty(IFlowFieldVectorModifier modifier)
        {
            ...
            FlowFieldCellRect dirty = entry.SurfaceState != null
                ? entry.SurfaceState.InfluenceRect
        ...
        internal FlowFieldModifierRegistryResult MarkAreaDirty(IFlowFieldVectorModifier modifier)
        {
            ...
            return new FlowFieldModifierRegistryResult(
                true,
                false,
                false,
                entry.SurfaceState != null
                    ? entry.SurfaceState.InfluenceRect
                    : FlowFieldCellRect.Invalid);
        }
        ...
        {
            for (int i = 0; i < _entries.Count; i++)
                _entries[i].SurfaceState?.Release();
            _entries.Clear();
            IsComposing = false;
        }
        ...
        private FlowFieldModifierRegistryResult UnregisterImmediately(IFlowFieldVectorModifier modifier)
        {
            ...
            for (int i = _entries.Count - 1; i >= 0; i--)
            {
                ...
                dirty = FlowFieldCellRect.Union(
                    dirty,
                    _entries[i].SurfaceState != null
                        ? _entries[i].SurfaceState.InfluenceRect
                        : FlowFieldCellRect.Invalid);
                _entries[i].SurfaceState?.Release();
                _entries.RemoveAt(i);
                removed = true;
    ...
    /// <summary>
    /// Surface-only per-cell cache. It is deliberately allocated by the
    /// Surface modifier pipeline, not by the common registry or Volume3D.
    /// </summary>
    internal sealed class FlowFieldSurfaceModifierState
    {
        ...
        {
            InfluenceMask = null;
            InfluenceScratch = null;
            InfluenceIndices.Clear();
            InfluenceIndicesBuilt = false;
            InfluenceRect = FlowFieldCellRect.Invalid;
        }
    }

    /// <summary>
    /// Immutable modifier inputs captured for one Volume build. Registry
    /// metadata is copied beside the value snapshot so a build never reads a
    /// live component after it has been accepted.
    /// </summary>
    internal readonly struct FlowFieldModifierWorkItem
    {
        internal IFlowFieldModifierSnapshot Snapshot { get; }
        internal Collider InfluenceCollider { get; }
        internal int Priority { get; }
        internal int Revision { get; }

        internal FlowFieldModifierWorkItem(
            IFlowFieldModifierSnapshot snapshot,
            Collider influenceCollider,
            int priority,
            int revision)
        {
