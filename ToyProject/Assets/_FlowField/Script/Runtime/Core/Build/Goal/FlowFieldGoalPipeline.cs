...
namespace Common.FlowField
{
    ...
    internal static class FlowFieldGoalPipeline
    {
        ...
        {
            FlowFieldGoalSolveInput request = resolution.IsValid
                ? new FlowFieldGoalSolveInput(
                    true,
                    resolution.SourceCellIndex,
                    resolution.InfluenceRadius)
                : FlowFieldGoalSolveInput.None;
            ...
            {
                workspace.ClearGoal();
                tracker.Record(FlowFieldGoalSolveInput.None, FlowFieldGoalBuildStatus.Invalid);
    ...
    internal readonly struct FlowFieldGoalSolveInput
    {
        public bool HasGoal { get; }
        public int SourceCellIndex { get; }
        public float InfluenceRadius { get; }

        internal FlowFieldGoalSolveInput(bool hasGoal, int sourceCellIndex, float influenceRadius)
        {
            HasGoal = hasGoal;
            SourceCellIndex = sourceCellIndex;
            InfluenceRadius = influenceRadius;
        ...
        internal static FlowFieldGoalSolveInput None
            => new FlowFieldGoalSolveInput(false, -1, 0f);
    }
    ...
    internal sealed class FlowFieldGoalTracker
    {
        ...
        private FlowFieldGoalSolveInput _lastObservedRequest;
        private bool _missingWalkableWarningIssued;

        internal bool HasBuiltGoal => _lastBuiltHadGoal;
        internal int LastBuiltSourceCellIndex => _lastBuiltSourceCellIndex;

        internal bool MatchesLastBuild(in FlowFieldGoalSolveInput request)
            => _hasBuiltRequest
            ...
            float influenceRadius)
        {
            ...
            {
                FlowFieldGoalSolveInput none = FlowFieldGoalSolveInput.None;
            ...
            FlowFieldGoalSolveInput request = new FlowFieldGoalSolveInput(
                true,
                resolution.SourceCellIndex,
                resolution.InfluenceRadius);
        ...
        internal bool HasChanged(in FlowFieldGoalSolveInput request)
        {
            if (!request.HasGoal)
        ...
        internal void Record(in FlowFieldGoalSolveInput request, FlowFieldGoalBuildStatus status)
        {
        ...
        internal void Clear()
        {
            ...
            _lastObservedRequest = FlowFieldGoalSolveInput.None;
            _missingWalkableWarningIssued = false;
        }

        private static bool RequestsEqual(
            in FlowFieldGoalSolveInput left,
            in FlowFieldGoalSolveInput right)
