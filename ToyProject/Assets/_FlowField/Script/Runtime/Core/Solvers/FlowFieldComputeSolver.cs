...
namespace Common.FlowField
{
    ...
    internal readonly struct FlowFieldComputeRequest
    {
        ...
        internal bool[] VolumeBlocked { get; }
        internal bool[] VolumeInfluence { get; }
        internal uint[] VolumeTopology { get; }
            ...
            int version)
        {
            ...
            Workspace = workspace;
            VolumeBlocked = null;
            VolumeInfluence = null;
            VolumeTopology = null;
            SpaceMode = FlowFieldSpaceMode.Surface2D;
            GoalIndex = goalIndex;
            ...
            int version)
        {
            ...
            VolumeInfluence = volumeInfluence;
        ...
        internal bool IsVolume => SpaceMode == FlowFieldSpaceMode.Volume3D;
    }
    ...
    internal sealed class FlowFieldComputeSolver : IDisposable
    {
        ...
        // Logical payload bytes owned by this solver. This deliberately
        // counts ComputeBuffer, upload and persistent readback storage, not
        // Unity/driver allocator overhead which is outside the module budget.
        ...
        {
            _shader = shader != null
                ? UnityEngine.Object.Instantiate(shader)
                : null;
            if (_shader != null)
            ...
            Action<FlowFieldComputeRequest, FlowFieldComputeFailureKind, Exception> failed)
        {
            if (_disposed || _running || !IsSupported || completed == null || failed == null)
                return false;
            bool validInput = request.IsVolume
                ? request.Grid.IsValid
                    && request.Grid.SpaceMode == FlowFieldSpaceMode.Volume3D
                    && request.VolumeBlocked != null
                    && request.VolumeInfluence != null
                    && request.VolumeTopology != null
                    && request.VolumeBlocked.Length == request.Grid.CellCount
                    && request.VolumeInfluence.Length == request.Grid.CellCount
                    && request.VolumeTopology.Length == request.Grid.CellCount
                : request.Grid.IsValid
            ...
            try
            {
                // Start is never allowed to synchronously wait for a previous
                // GPU readback. If one is still in flight, the caller must
                // use the managed fallback and try again on a later build.
                if (_pendingControl || _pendingOutput)
                    return false;
                EnsureBuffers(request.Grid.CellCount, request.IsVolume);
                ...
                _shader.SetInt("_Height", request.Grid.Height);
                _shader.SetInt("_Depth", request.Grid.Depth);
                _shader.SetInt("_SpaceMode", request.IsVolume ? 1 : 0);
        ...
        private void RunBatch()
        {
            ...
            _controlRequest = AsyncGPUReadback.Request(
                _control,
                readback => OnControlReadback(readback, operationId));
        }

        private void OnControlReadback(AsyncGPUReadbackRequest readback, int operationId)
        {
            if (!_running || operationId != _operationId)
                return;
            _pendingControl = false;
            ...
            try
            {
                ...
                if (active == 0u)
                {
                    BuildDirectionsAndReadback(operationId);
                    return;
                }
        ...
        private void BuildDirectionsAndReadback(int operationId)
        {
            try
            {
                ...
                _pendingOutput = true;
                _outputRequest = AsyncGPUReadback.RequestIntoNativeArray(
                    ref target,
                    _output,
                    readback => OnOutputReadback(readback, operationId));
            }
            catch (Exception exception)
            {
                Fail(FlowFieldComputeFailureKind.Error, exception);
        ...
        private void OnOutputReadback(AsyncGPUReadbackRequest readback, int operationId)
        {
            if (!_running || operationId != _operationId)
                return;
            _pendingOutput = false;
        ...
        private void EnsureBuffers(int cellCount, bool volume)
        {
            if (_cellCount == cellCount && _cellState != null && _volumeBuffers == volume)
                return;
            ...
            try
            {
                ...
                // Volume3D never reads surface height/normal data. Keep one
                // dummy binding for the shared shader instead of allocating a
                // cell-sized height/normal buffer.
                _surfaceData = new ComputeBuffer(volume ? 1 : cellCount, 16, ComputeBufferType.Structured);
                ...
                _surfaceUpload = new GpuSurfaceCell[volume ? 1 : cellCount];
                ...
                _volumeBuffers = volume;
                AllocatedBytes = EstimateAllocationBytes(cellCount, volume);
            }
            catch
            {
                ReleaseBuffers();
        ...
        private void UploadInputs(in FlowFieldComputeRequest request)
        {
            ...
            {
                uint flags = 0u;
                bool valid = request.IsVolume || request.Surface.IsSurfaceValid(index);
                bool blocked = request.IsVolume
                    ? request.VolumeBlocked[index]
                    : request.Workspace.Blocked[index];
                bool influence = request.IsVolume
                    ? request.VolumeInfluence[index]
                    : request.Workspace.InfluenceMask[index];
                uint topology = request.IsVolume
                    ? request.VolumeTopology[index]
                    : request.Workspace.TopologyMasks[index];
                if (valid)
                    flags |= VALID;
                if (blocked)
                    flags |= BLOCKED;
        ...
        private void ReleaseBuffers()
        {
            ...
            if (_readbackB.IsCreated)
                _readbackB.Dispose();
            _cellCount = 0;
            _volumeBuffers = false;
            AllocatedBytes = 0;
        }

        internal static long EstimateAllocationBytes(int cellCount, bool volume)
        {
            if (cellCount <= 0)
                return 0;
            long surfaceCount = volume ? 1L : cellCount;
            // cell state, two frontiers, output, upload state and two
            // readbacks are 72 bytes per cell. Surface data and its upload
            // are one 16-byte element per cell for Surface2D and one dummy
            // element for Volume3D. The constants cover control/args buffers.
            return checked(
                72L * cellCount
                + 32L * surfaceCount
                + 28L);
