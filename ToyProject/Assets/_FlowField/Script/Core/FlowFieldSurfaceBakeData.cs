...
namespace Common.FlowField
{
    public sealed class FlowFieldSurfaceBakeData : ScriptableObject
    {
        ...
        internal Vector3 GetSurfaceNormal(int index)
        {
            ...
            if (!FlowFieldGridSpace.IsFinite(normal)
                || normal.sqrMagnitude <= FlowFieldVectorUtility.DIRECTION_EPSILON_SQR)
                throw new ArgumentException("Surface bake contains an invalid normal.", nameof(index));
        ...
        /// Bake 결과는 장애물 조회를 수행할 대상이 없는 정상 결과로 false입니다.
        /// </summary>
        /// <returns>하나 이상의 유효한 Surface 셀이 있으면 true, 없으면 false입니다.</returns>
        ...
        {
            if (!settings.IsValid)
                throw new ArgumentException("Surface bake settings are invalid.", nameof(settings));
            if (result == null)
                throw new ArgumentNullException(nameof(result));
            if (!result.IsValidFor(settings.Grid.CellCount))
                throw new ArgumentException("Surface bake result does not match the grid cell count.", nameof(result));
