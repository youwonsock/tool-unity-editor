...
namespace Common.FlowField
{
    internal static class FlowFieldHierarchicalGoalSolver
    {
            ...
            out int resolvedGoalIndex)
        {
            ...
            if (workspace.Capacity != fineGrid.CellCount)
                throw new ArgumentException("Workspace capacity must match the fine grid.", nameof(workspace));
            if (!fineGrid.IsLocalInBounds(requestedGoalX, requestedGoalZ))
                throw new ArgumentOutOfRangeException(nameof(requestedGoalX));
            ...
            if (coarse != null && !coarse.HasValidData)
                throw new ArgumentException("Coarse topology data is invalid.", nameof(coarse));
        ...
        private static void BuildFineInfluenceMask(
            ...
            int fineRingCoarseRadius)
        {
            if (fineRingCoarseRadius < 0)
                throw new ArgumentOutOfRangeException(nameof(fineRingCoarseRadius));
