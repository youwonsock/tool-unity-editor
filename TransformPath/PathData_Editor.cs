...
namespace Supercent.Common.TransformPath
{
    public partial class PathData
    {
        ...
        [SerializeField] [Range(0f, 1f)] private float _pointSize = DEFAULT_POINT_SIZE;
        [SerializeField] [Range(0f, 1f)] private float _samplePointSize = 0.0f;
        ...
        private Vector3[] _lastPathPointPositions;
        ...
        private void OnValidate()
        {
            ...
            if (_lastPathPointCount != _pathPoints.Count)
            {
                _needsUpdate = true;
                return;
            }
        ...
        private void UpdatePathIfNeeded()
        {
            ...
            if (_lastPathPointPositions == null || _lastPathPointPositions.Length != _pathPoints.Count)
            {
                needsInit = true;
            }
            ...
            else if (HasPathPointPositionChanged())
            {
                needsInit = true;
            }
            ...
            if (needsInit)
            {
                UpdateLastPathPointPositions();
                Init(true);
                _needsUpdate = false;
            }
        }

        private bool HasPathPointPositionChanged()
        {
            if (_lastPathPointPositions == null || _pathPoints == null)
                return true;
            ...
            for (int i = 0; i < _pathPoints.Count; i++)
            {
                ...
                {
                    Vector3 currentPosition = _pathPoints[i].position;
                    if (Vector3.Distance(currentPosition, _lastPathPointPositions[i]) > POSITION_THRESHOLD)
                        return true;
                }
            }
        ...
        private void UpdateLastPathPointPositions()
        {
            if (_pathPoints == null)
                return;

            _lastPathPointPositions = new Vector3[_pathPoints.Count];
            for (int i = 0; i < _pathPoints.Count; i++)
            {
                if (_pathPoints[i] != null)
                    _lastPathPointPositions[i] = _pathPoints[i].position;
                else
                    _lastPathPointPositions[i] = Vector3.zero;
            }
        }
        #endregion
        ...
        private class PathDataEditor : Editor
        {
            ...
            public override void OnInspectorGUI()
            {
                ...
                if (GUILayout.Button(new GUIContent("Refresh Path Points from Children", $"{PATH_POINT_NAME_PREFIX}로 시작하는 이름의 자손 Transform을 형제 순으로 _pathPoints에 반영합니다.")))
                    RefreshPathPointsFromChildren(pathData);
            ...
            private void SnapToGround(PathData pathData)
            {
                ...
                {
                    EditorUtility.SetDirty(pathData);
                    PathDataInitialization.Initialize(pathData, forceReinit: true);
            ...
            private void RefreshPathPointsFromChildren(PathData pathData)
            {
                if (pathData == null)
                    return;

                Undo.RecordObject(pathData, "Refresh Path Points From Children");
