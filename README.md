...
각 쇼케이스는 `Assets/_기능명/Script`, `Scene`, `Prefab`, `Settings`로 독립 보관하며, 실행 가능한 Demo 루트는 해당 기능의 `Prefab`에 함께 저장합니다. 샘플·기능 검증 클래스는 각 기능의 `Script` 내부 샘플 영역에 모아 시스템 코드와 분리합니다.
